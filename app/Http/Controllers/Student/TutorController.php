...
class TutorController extends Controller
{
    public function ourTutor(Request $request)
    {
        ...
        return view('student.tutor.find_tutor', get_defined_vars());
    }

    public function loadTutorProfile($id = null)
    {
        $tutor = User::find($id);
        return view('ajax.student.load_tutor_profile', get_defined_vars());
    }
    ...
    public function requestTutor(Request $request)
    {
        ...
        if ($has_subscription) {
            ...
            if ($this->subscribedRequest((object)$data)) {
                return redirect()->route('student.dashboard')->with('success', 'Your request has been submitted to tutor');
            } else {
    ...
    public function tutorRequest()
    {
        ...
        return view('student.tutor.student_request', get_defined_vars());
    }
    ...
    public function recurringPayment($id = null)
    {
        ...
        return redirect()->route('student.payment.form');
    }
    ...
    public function requestTutorStatus($action = null, $id = null)
    {
        ...
        if ($user->time_zone == $tutor->time_zone) {
            $start_time = Carbon::parse($t_req->active_date." ".explode("-", explode(", ", $t_req->slot)[0])[0]);
            $now = Carbon::now();
        } else {
    ...
    public function subscribedRequest($data)
    {
        // dd($data);
        $user = auth()->user();
        $remaining = SubscribePlan::where('user_id', $user->id)->where('status', 'active')->first()->remaining_hour;
