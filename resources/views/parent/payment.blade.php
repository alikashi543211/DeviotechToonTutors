                ...
                <div class="card">
                    <div class="card-body">
                        @if ($t_req->has_subscription)
                            <p class="font-weight-bold text-center">You don't have enough hours remaining <br> <a href="{{ route('parent.packages') }}" class="text-primary">Click Here</a> to Buy Package </p>
                            <h2 class="text-center">OR</h2>
                        @endif
                        <p class="font-weight-bold text-center">Pay tutor fee of ${{ $rate }}</p>
                        <form action="{{ route('parent.payment.save') }}" method="POST" id="payment-form" class=" text-center">
                            @csrf
